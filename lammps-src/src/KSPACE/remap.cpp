...
{
  LAMMPS_NS::Timer::phase_marker_hook("KSPACE_FFT_COMM");
  ...
  if (!plan->usecollective) {
    ...
    for (isend = 0; isend < plan->nsend; isend++) {
      ...
      LAMMPS_NS::WsePlanKspace::instance().message(
        "fft_remap",plan->send_proc[isend],plan->send_size[isend],
        sizeof(FFT_SCALAR));
  ...
  // use All2Allv collective for remap communication
    ...
    if (plan->commringlen > 0) {
      ...
      LAMMPS_NS::WsePlanKspace::instance().collective(
        "fft_remap","alltoallv",sendBufferSize,sizeof(FFT_SCALAR));
  ...
  LAMMPS_NS::Timer::phase_marker_hook("KSPACE_FFT_CALC");
}
