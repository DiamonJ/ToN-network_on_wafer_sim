...
void PPPM::compute(int eflag, int vflag)
{
  ...
  WsePlanKspace::instance().begin(me,nprocs,update->setupflag,
  ...
  timer->mark_subphase("KSPACE_GRID_FORWARD");
  ...
  timer->mark_subphase("KSPACE_FORCE");
  fieldforce();
  ...
  timer->mark_subphase("KSPACE_REDUCE");
  const double qscale = qqrd2e * scale;

  if (eflag_global) {
    double energy_all;
    WsePlanKspace::instance().collective("kspace_reduce","allreduce",
                                         1,sizeof(double));
