...
class Timer : protected Pointers {
 public:
  ...
  // phase trace for DUMPI correlation:
  // writes phase-switch and sub-phase marker timestamps to a CSV file
  // (only rank 0, CLOCK_MONOTONIC ns since boot)

  void enable_phase_trace(const char *path);
  void disable_phase_trace();

  // write a named marker inside the current timer phase
  // (e.g. Kspace sub-stages in PPPM::compute)

  void mark_subphase(const char *name);

  // global hook so C-style code (fft3d.cpp) can emit sub-phase markers
  // without holding a LAMMPS pointer; set by rank 0's Timer

  static void (*phase_marker_hook)(const char *);
  static void default_phase_marker(const char *);
  // under SMPI (SMPI_PRIVATIZATION=0) every rank is a coroutine in one
  // process/thread sharing globals; the hook filters by MPI rank at call
  static class Timer *_hook_owner;
